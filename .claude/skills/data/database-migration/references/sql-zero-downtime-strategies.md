# SQL Zero-Downtime Migration Strategies

Production-ready SQL migration patterns for PostgreSQL, MySQL, and SQL Server with zero-downtime deployment strategies.
...
CREATE TABLE v2_orders (
    ...
    CONSTRAINT fk_v2_orders_customer FOREIGN KEY (customer_id) REFERENCES customers(id),
    CONSTRAINT chk_v2_orders_amount CHECK (total_amount >= 0)
);

-- Step 2: Dual-write synchronization trigger
...
## Online Schema Change (Safe NOT NULL)
...
ALTER TABLE large_table
    ADD CONSTRAINT chk_new_field_not_null CHECK (new_field IS NOT NULL) NOT VALID;
ALTER TABLE large_table VALIDATE CONSTRAINT chk_new_field_not_null;
```
...
def validate_pre_migration(db_connection):
    ...
    if null_check[0]['null_count'] > 0:
        checks.append({
            'check': 'null_values', 'status': 'FAILED',
            'severity': 'CRITICAL',
    ...
    if duplicate_check:
        checks.append({
            'check': 'duplicates', 'status': 'FAILED',
            'severity': 'CRITICAL',
...
def validate_post_migration(db_connection, migration_spec):
    ...
    for table in migration_spec['affected_tables']:
        ...
        validations.append({
            'check': 'row_count', 'table': table['name'],
            'expected': table['expected_count'], 'actual': actual_count,
...
WHERE tablename = 'large_table' AND indexname NOT LIKE '%pkey%';

-- Drop indexes
DO $$
DECLARE idx_record RECORD;
BEGIN
    FOR idx_record IN SELECT indexname FROM migration_indexes LOOP
...
DECLARE idx_record RECORD;
BEGIN
    FOR idx_record IN SELECT indexdef FROM migration_indexes LOOP
