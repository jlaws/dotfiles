...
For deep frontend guidance, delegate to the `frontend-engineer` agent via the Task tool, passing the diagnostic findings above and the request. It loads its skills (language-testing-patterns, output-completeness) and the `references/frontend/` library, then returns specific guidance. Verify its output before presenting.
