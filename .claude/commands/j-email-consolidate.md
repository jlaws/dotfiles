...
Load skill `output-completeness` via the Skill tool -- the digest must account for every source with real signal; no silently dropped items, no truncated sections.
