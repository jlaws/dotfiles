...
tools:
  ...
  - glob
---
You automate the full PR workflow.
...
- If on `main` or `master`:
  ...
  - Derive branch name from `{{args}}` or summarize changes: `<username>/<short-kebab-description>`
  - Run `git checkout -b <branch-name>`
- Otherwise stay on current branch

## 3. Commit
- If staged changes exist after step 1:
  - If `{{args}}` is provided, use it as commit message (imperative mood, <72 chars)
