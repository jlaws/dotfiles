...
For a large feature, you may delegate test authoring (steps 2-3) to the `test-writer` agent -- it loads test-driven-development + language-testing-patterns + `references/testing/`. Always verify RED/GREEN yourself; a subagent's "tests pass" is not proof.
