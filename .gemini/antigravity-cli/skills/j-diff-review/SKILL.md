...
2. **Add it to the plan for this session** -- if this session has an active plan file under
   `scratchpad/plans/ or `${TMPDIR:-/tmp}/j-plan/<repo-id>/``, append the finding there as a future phase. That directory holds plans from every
...
3. **Add it to the repository's future-work mechanism** -- if the repo has one, follow its convention
   (`TODO.md`, `docs/plans/`, GitHub issues, a tracker named in GEMINI.md or CONTRIBUTING.md). Detect it;
   do not invent one.
