...
## Step 0: Discover and select the plan

Discover all regular, non-symlink plan files under:
1. `scratchpad/plans/`
2. `${TMPDIR:-/tmp}/j-plan/<repo-id>/`

Display full paths and modification times, even when there is only one.
You MUST NOT execute a discovered plan without confirmation from the user.
