...
For deep DevOps guidance, delegate to the `devops-engineer` agent, passing the diagnostic findings above and the request. It loads its skills and the `references/devops/` library (plus `references/workflow/release-versioning`), then returns specific guidance. Verify its output before presenting.
