...
- **Then carry it to done without checking in.** Do not stop at the PR and do not ask the user to review it. Run the loop: open the PR -> `$cmd-j-diff-review` it -> fix every finding -> wait for CI -> merge when green -> `$cmd-j-next` for the following chunk -> repeat. Report at the end of a chunk, not to get permission to continue.
