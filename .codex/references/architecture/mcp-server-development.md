...
|-----------|----------|------|------|
| **stdio** | Local tools, CLI integrations, Codex | Simple, no networking, secure | Single client only |
...
**Decision rule**: Use stdio for local dev tools and Codex integrations. Use streamable HTTP for production remote servers. SSE is legacy but still widely supported.
