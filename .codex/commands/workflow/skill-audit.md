---
name: skill-audit
description: "Audit the Codex knowledge base — skills, commands, agents, references, config, rules, and cross-references for conformance and integrity. Use when creating or modifying any Codex asset to validate compliance. Do NOT use for quick checks (inspect files directly instead)."
...
Full conformance audit of all assets under `.codex/` and `.agents/skills/`.
...
| Commands | `.codex/commands/{category}/{name}.md` | Markdown files |
| Agents | `.codex/agents/{name}.toml` | TOML files |
| References | `.codex/references/{category}/{name}.md` | Markdown files |
| Config | `.codex/AGENTS.md`, `.codex/config.toml`, `.codex/rules/*.rules` | Fixed paths |
...
For each `.toml` file under `.codex/agents/`:

**Structural**

| # | Check | Sev | Rule |
|---|-------|-----|------|
| AG-S1 | Filename is kebab-case (`.toml` extension) | FAIL | Naming convention |
...
| # | Check | Sev | Rule |
|---|-------|-----|------|
| AG-F1 | TOML parses successfully | FAIL | Required |
| AG-F2 | `name` field exists | FAIL | Required |
| AG-F3 | `name` matches filename (without `.toml`) | FAIL | Must be identical |
| AG-F4 | `description` field exists | FAIL | Required |
| AG-F5 | `description` under 1024 chars | WARN | Keep concise |
| AG-F6 | `developer_instructions` field exists | FAIL | Required |
| AG-F7 | Optional config fields are valid TOML | WARN | Invalid fields ignored at runtime |
...
| AG-C2 | If instructions reference skills, each skill resolves to an existing SKILL.md | FAIL | Broken skill ref |
| AG-C3 | If instructions reference `.codex/references/`, each path resolves | WARN | Broken reference link |

---

#### 2C. Command Checks

For each `.md` file under `.codex/commands/`:
...
| CM-C4 | If body invokes a subagent, agent exists under `.codex/agents/` | WARN | Broken agent ref |

---

#### 2D. Reference Checks

For each `.md` file under `.codex/references/`:
...
| # | Check | Sev | Rule |
|---|-------|-----|------|
| CF-C1 | `.codex/AGENTS.md` exists | FAIL | Required config |
...
| # | Check | Sev | Rule |
|---|-------|-----|------|
| CF-S1 | `.codex/config.toml` exists | WARN | Expected for configured projects |
...
| XR-2 | Agent instruction reference paths (`.codex/references/...`) resolve | WARN | Broken ref link |
...
- For cross-ref checks, normalize paths (strip `.codex/`, `.md`, leading/trailing slashes)
