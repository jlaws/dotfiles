...
description: "Analyze recent conversation history to improve AGENTS.md guidance — find violated instructions, missing patterns, and outdated rules. Use when tuning Codex behavior or after a batch of sessions. Do NOT use for quick questions (edit files directly instead)."
argument-hint: "<scope: global|local|number|empty>"
---

Scope: $ARGUMENTS

If no arguments provided, analyze both global and local AGENTS.md guidance against the last 20 conversations.

Use the multi-agent development methodology below with AGENTS.md review agents to analyze conversation history.
