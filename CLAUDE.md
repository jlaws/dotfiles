...
- A shared reference must not cite `.claude/references/`: Codex and Gemini have no such checkout, so the pointer resolves nowhere. Cite `references/<category>/<file>.md`, which is also the only form `audit.py`'s XR-7 can validate
- `tests/test_agent_config.py` owns anything spanning two trees, not just parity: Codex agents must declare `name`/`description`/a non-empty `developer_instructions`, Gemini agents must be `model: inherit`, shared non-command skills must be byte-identical between `.agents/` and `.gemini/antigravity-cli/`, and no tracked asset in any tree may name a script the repo does not ship. `audit.py` is Claude-tree only by its own docstring, so cross-tree checks belong in the test file
