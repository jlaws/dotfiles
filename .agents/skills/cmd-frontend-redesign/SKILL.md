...
3. **Identify fonts**: Search for font imports in CSS, `_document.tsx`, `layout.tsx`, or `index.html`.
4. **Scan for AI fingerprints**: Search for `Inter`, `Roboto`, gradient classes, `grid-cols-3`, hardcoded `#000000`.
5. **Scope the target**: If the user names a component/page, read it directly. Otherwise scan `src/`, `app/`, `pages/` for the largest/most visible UI surfaces.

Then apply the following redesign framework:
...
Check against the fingerprint checklist:
...
Read `.agents/references/frontend/premium-design-aesthetics` and `.agents/references/frontend/design-system-patterns` before producing recommendations (if available).

Target: the component, page, or directory specified by the user.
