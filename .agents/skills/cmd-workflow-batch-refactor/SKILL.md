---
name: cmd-workflow-batch-refactor
...
Refactoring: the user's provided input
...
For comprehensive debt audits of large codebases, dispatch parallel search tasks:

1. **code-debt-analyst** (parallel search) — Duplication, complexity, code smell inventory
2. **arch-debt-analyst** (parallel search) — Boundary violations, dependency analysis, pattern drift
3. **test-debt-analyst** (parallel search) — Coverage gaps, flaky tests, missing integration tests
4. **infra-debt-analyst** (parallel search) — Deployment gaps, monitoring holes, dependency health
