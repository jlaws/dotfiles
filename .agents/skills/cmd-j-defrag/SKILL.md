...
Load skills `refactoring-and-debt` and `code-quality` before doing anything else. Defragment the codebase: scan for fragmentation and fix it in verified batches so the next thing built lands in a more coherent codebase.

## Phase 1 — Scope

Resolve the scan scope: if the user provided input, limit to that path or subsystem. Else scan the whole repo.
