---
name: cmd-j-next
description: "Use when invoking the j-next workflow."
...
Scope stops at this part's PR boundary. Later phases are the next run of this workflow, not this one.

Maintain the plan file's living-document sections as you go — `## Progress` with commit SHAs, `## Decision Log`, `## Surprises & Discoveries`. They are how the next run of this workflow finds its place.
...
- **skill:cmd-j-execute-plan** -- executes a plan from its first part; this workflow resumes at a later PR boundary
- **skill:cmd-j-rebase** -- refreshes an already-open PR; this workflow opens the next one
