...
4. **Get file tree overview** of the target scope (if the user's provided input specifies a component/directory, scope to that; otherwise scope to the full project).
...
For comprehensive threat modeling of complex systems with multiple trust boundaries and compliance requirements, dispatch parallel search tasks:

1. **stride-analyst** (parallel search) — STRIDE category analysis per component and trust boundary
2. **attack-tree-builder** (parallel search) — Attack tree construction from STRIDE findings, path prioritization
3. **sast-scanner** (general-purpose) — SAST rule execution, custom Semgrep rule generation, false positive triage
4. **compliance-mapper** (parallel search) — Map findings to PCI-DSS, HIPAA, GDPR, OWASP ASVS controls
...
Apply the above methodology to analyze: the user's provided input
