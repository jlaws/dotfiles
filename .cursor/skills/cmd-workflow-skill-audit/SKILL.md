---
name: cmd-workflow-skill-audit
description: "Audit the .cursor/ knowledge base — skills, commands, agents, references, config, and cross-references for conformance and integrity. Use when creating or modifying any .cursor/ asset to validate compliance. Do NOT use for quick checks (inspect files directly instead)."
...
Full conformance audit of all assets under `.cursor/`.
...
|------|----------|---------|
| Skills | `.cursor/skills/{category}/{name}/SKILL.md` | Folder with SKILL.md |
| Commands | `.cursor/commands/{category}/{name}.md` | Markdown files |
| Agents | `.cursor/agents/{name}.md` | Markdown files |
| References | `.cursor/references/{category}/{name}.md` | Markdown files |
| Config | `.cursor/CLAUDE.md`, `.cursor/settings.json`, `.cursor/settings.local.json` | Fixed paths |
...
For each skill directory under `.cursor/skills/`:
...
For each `.md` file under `.cursor/agents/`:
...
| AG-C3 | If body references `.cursor/references/`, each path resolves | WARN | Broken reference link |

---

#### 2C. Command Checks

For each `.md` file under `.cursor/commands/`:
...
| CM-C4 | If body invokes a subagent, agent exists under `.cursor/agents/` | WARN | Broken agent ref |

---

#### 2D. Reference Checks

For each `.md` file under `.cursor/references/`:
...
| # | Check | Sev | Rule |
|---|-------|-----|------|
| CF-C1 | `.cursor/CLAUDE.md` exists | FAIL | Required config |
...
| # | Check | Sev | Rule |
|---|-------|-----|------|
| CF-S1 | `.cursor/settings.json` exists | WARN | Expected for configured projects |
...
| XR-2 | Agent body reference paths (`.cursor/references/...`) resolve | WARN | Broken ref link |
...
- For cross-ref checks, normalize paths (strip `.cursor/`, `.md`, leading/trailing slashes)
